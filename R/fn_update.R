...
#' Update resource use tibble for gndr diffs
#' @description update_resc_use_tb_for_gndr_diffs() is an Update function that edits an object, while preserving core object attributes. Specifically, this function implements an algorithm to update resource use tibble for gndr diffs. Function argument resource_use_tb specifies the object to be updated. Argument fem_uids_chr provides the object to be updated. The function returns Resource use (a tibble).
#' @param resource_use_tb Resource use (a tibble)
#' @param fem_uids_chr Fem uids (a character vector), Default: 'AUS_SNR_F'
#' @param male_uids_chr Male uids (a character vector), Default: 'AUS_SNR_M'
#' @param original_fem_data_dbl Original fem data (a double vector), Default: c(0.2, 0.26666667)
#' @param new_fem_data_dbl New fem data (a double vector), Default: c(0.398, 0.306)
