...
#' @description calculate_expenditure() is a Calculate function that calculates a numeric value. Specifically, this function implements an algorithm to calculate expenditure. The function returns Expenditure (a double vector of length one).
#' @param resources_tb Resources (a tibble)
#' @param incld_areas_chr Incld areas (a character vector), Default: 'NA'
    ...
    n_ress_var_nm_1L_chr = "Unit_Qty_dbl", unit_cost_var_nm_1L_chr = "Unit_Cost_Dollars_dbl") 
{
    ...
    expenditure_dbl <- resources_tb %>% dplyr::pull(!!rlang::sym(n_ress_var_nm_1L_chr)) * 
