...
print_intvs_df <- function(intvs_df,
                           ...
                           use_rdocx_1L_lgl = T){
  intvs_df <- intvs_df %>%
    dplyr::select(c(names(intvs_df)[1:10],
                    names(intvs_df)[12],
                    names(intvs_df)[11],
                    names(intvs_df)[13:14]))
  intvs_df %>%
    ...
    dplyr::rename(Quantity = N_OOS_Per_Episode_dbl,
                  ...
                  Activity = Intervention_Name_chr,
                  Format = Intervention_Format_chr) %>%
    dplyr::select(-Domain_chr, - Intervention_Category_chr) %>%
    dplyr::mutate(Format = dplyr::case_when(Format == "F2F" ~ "Face to face", Format == "Provider only" ~ "Desk",T ~ Format)) %>%
    remove_col_nms_obj_sfcs(complete_cases_1L_lgl = T) %>%
    ready4show::print_table(output_type_1L_chr = output_type_1L_chr,
                            use_rdocx_1L_lgl = use_rdocx_1L_lgl,
...
print_resource_use_df <- function(resource_use_df,
  ...
  resource_use_df %>%
    ...
    dplyr::rename(Activity = Intervention,`Team Group` = Recipient,`Usage (percent)` = Proportion_Using_Service) %>%
    remove_col_nms_obj_sfcs(complete_cases_1L_lgl = T) %>%
    ready4show::print_table(output_type_1L_chr = output_type_1L_chr,
                            use_rdocx_1L_lgl = use_rdocx_1L_lgl,
                            caption_1L_chr = "Resource use",
